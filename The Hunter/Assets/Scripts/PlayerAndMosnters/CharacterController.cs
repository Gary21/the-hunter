...
public class CharacterController : MonoBehaviour
{
    ...
    private float _dashDoubleClickLeft = 0.0f;
    private float _dashDoubleClickRight = 0.0f;
    
    //Attack
    [SerializeField]private GameObject attackArea;
    [SerializeField]private float attackCooldown = 100.25f;
    ...
    [SerializeField] private Vector2 _groundCheckSize = new Vector2(5.0f, 0.03f);
    [Space(5)]
    ...
    private void Update()
    {
        ...
        {
            if(_dashDoubleClickLeft > 0.0f)
                OnDashInput();
            _dashDoubleClickLeft = 0.25f;
        }
        if (Input.GetKeyDown(KeyCode.RightArrow) || Input.GetKeyDown(KeyCode.D))
        {
            if(_dashDoubleClickRight > 0.0f)
                OnDashInput();
            _dashDoubleClickRight = 0.25f;
        }
        
        AttackHandler();

        _dashDoubleClickLeft -= Time.deltaTime;
        _dashDoubleClickRight -= Time.deltaTime;
        #endregion
        ...
        if (!IsDashing && !IsJumping)
        {
            ...
            if (Physics2D.OverlapBox(_groundCheckPoint.position, _groundCheckSize, 0, _groundLayer) && !IsJumping) //checks if set box overlaps with ground
            {
                ...
                isTouchingGround = true;//if so sets the lastGrounded to coyoteTime
                doubleJump = true;
            }
            else
            {
        ...
        playerAnimator.SetBool("isMoving", _moveInput.x != 0);
